...
node('local-docker') {
    ...
    stage('Upload linux binary') {
        withEnv(["GIT_COMMITTER_NAME=Jenkins", "GIT_COMMITTER_EMAIL=jenkins@minebest.com"]) {
            ...
            nexusArtifactUploader artifacts: [[artifactId: "${project}-${project_type}", classifier: '', file: "dist/${project}-${tag}-x86_64.AppImage", type: "${project_ext}"]], credentialsId: 'jenkins-rw-nexus', groupId: '', nexusUrl: "${nexus_url}", nexusVersion: 'nexus3', protocol: 'https', repository: 'miningcityv2', version: "${tag}"            
        }
    }
